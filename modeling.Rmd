...
for (m in metrics) {
  ...
  formula_lmer <- as.formula(paste0(m, " ~ season + year + (1|strata) + silt_clay_pct+ tot_perc_loss + total_count"))
  formula_lm   <- as.formula(paste0(m, " ~ season + year + strata + silt_clay_pct + tot_perc_loss +  total_count "))
...
modelsummary(
  list("Linear Model" = lm_mod[['shannon_index']], "Linear Mixed Model" = mod[['shannon_index']]),
  statistic = 'p.value',
  stars = TRUE,
...
mod_gamm <- gamm(shannon_index~ season + year  + s(tot_perc_loss)+ s(silt_clay_pct) + s(lat, lon),
